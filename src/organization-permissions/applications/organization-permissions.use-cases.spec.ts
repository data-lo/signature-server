...
import { GetOrganizationPermissionsUseCase } from './get-organization-permissions.use-case';
import { CreateOrganizationPermissionUseCase } from './create-organization-permission.use-case';
import { UpdateOrganizationPermissionUseCase } from './update-organization-permission.use-case';
import { DeleteOrganizationPermissionUseCase } from './delete-organization-permission.use-case';
import { GetMemberPermissionsUseCase } from './get-member-permissions.use-case';
import { AssignMemberPermissionsUseCase } from './assign-member-permissions.use-case';
...
/**
 * Se montan los casos de uso sobre el `OrganizationPermissionsService` de verdad, con los
 * repositorios simulados: lo que estas pruebas cuidan es justamente la composición —que la
 * comprobación de administrador ocurra antes de tocar nada, que el catálogo se valide antes de
 * asignar—, y eso desaparecería si el servicio fuera también un doble.
 */
describe('casos de uso de permisos de organización', () => {
  ...
  let getOrganizationPermissions: GetOrganizationPermissionsUseCase;
  let createOrganizationPermission: CreateOrganizationPermissionUseCase;
  let updateOrganizationPermission: UpdateOrganizationPermissionUseCase;
  let deleteOrganizationPermission: DeleteOrganizationPermissionUseCase;
  let getMemberPermissions: GetMemberPermissionsUseCase;
  let assignMemberPermissions: AssignMemberPermissionsUseCase;
  ...
  beforeEach(async () => {
    ...
    getOrganizationPermissions = module.get(GetOrganizationPermissionsUseCase);
    createOrganizationPermission = module.get(
      CreateOrganizationPermissionUseCase,
    );
    updateOrganizationPermission = module.get(
      UpdateOrganizationPermissionUseCase,
    );
    deleteOrganizationPermission = module.get(
      DeleteOrganizationPermissionUseCase,
    );
    getMemberPermissions = module.get(GetMemberPermissionsUseCase);
    assignMemberPermissions = module.get(AssignMemberPermissionsUseCase);
  });

  describe('GetOrganizationPermissionsUseCase', () => {
    it('retorna el catálogo si el llamador es ADMIN activo de la organización', async () => {
      ...
      const result = await getOrganizationPermissions.execute(
        'owner-1',
        'org-1',
      );
  ...
  describe('CreateOrganizationPermissionUseCase', () => {
    it('crea el permiso si el llamador es ADMIN activo de la organización', async () => {
      ...
      const result = await createOrganizationPermission.execute(
        'owner-1',
        'org-1',
    ...
    it('lanza ConflictException si ya existe un permiso con ese nombre en la organización', async () => {
      ...
      await expect(
        createOrganizationPermission.execute('owner-1', 'org-1', {
          name: 'Aprobar documentos',
        }),
  ...
  describe('UpdateOrganizationPermissionUseCase', () => {
    it('lanza NotFoundException si el permiso no existe en esa organización', async () => {
      ...
      await expect(
        updateOrganizationPermission.execute(
          'owner-1',
          'org-1',
          'missing-perm',
          { isActive: false },
    ...
    it('actualiza nombre y/o estatus cuando el permiso existe', async () => {
      ...
      await updateOrganizationPermission.execute('owner-1', 'org-1', 'perm-1', {
        isActive: false,
      });
    ...
    it('lanza ConflictException al renombrar a un nombre ya usado por otro permiso de la organización', async () => {
      ...
      await expect(
        updateOrganizationPermission.execute('owner-1', 'org-1', 'perm-1', {
          name: 'Ver reportes',
        }),
      ).rejects.toThrow(ConflictException);
      expect(organizationPermissionRepository.update).not.toHaveBeenCalled();
  ...
  describe('DeleteOrganizationPermissionUseCase', () => {
    it('lanza NotFoundException si el permiso no existe en esa organización', async () => {
      accountRepository.findOne.mockResolvedValue(adminMembership());
      organizationPermissionRepository.findOne.mockResolvedValue(null);

      await expect(
        deleteOrganizationPermission.execute(
          'owner-1',
          'org-1',
          'missing-perm',
        ),
    ...
    it('elimina el permiso cuando existe (la cascada limpia sus asignaciones)', async () => {
      ...
      const result = await deleteOrganizationPermission.execute(
        'owner-1',
        'org-1',
        'perm-1',
      );
  ...
  describe('GetMemberPermissionsUseCase', () => {
    it('retorna los IDs de permisos actualmente asignados al miembro', async () => {
      ...
      const result = await getMemberPermissions.execute('owner-1', 'member-1');
    ...
    it('lanza NotFoundException si la membresía no existe', async () => {
      ...
      await expect(
        getMemberPermissions.execute('owner-1', 'missing-member'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('lanza ForbiddenException si el llamador no es ADMIN de esa organización', async () => {
      ...
      await expect(
        getMemberPermissions.execute('intruder', 'member-1'),
      ).rejects.toThrow(ForbiddenException);
    });
  });

  describe('AssignMemberPermissionsUseCase', () => {
    it('lanza BadRequestException si algún permiso no pertenece al catálogo de la organización', async () => {
      ...
      await expect(
        assignMemberPermissions.execute('owner-1', 'member-1', [
          'perm-1',
          'perm-2',
        ]),
    ...
    it('reemplaza las asignaciones dentro de una transacción cuando los permisos son válidos', async () => {
      ...
      const result = await assignMemberPermissions.execute(
        'owner-1',
        'member-1',
        ['perm-1', 'perm-2'],
      );
    ...
    it('permite desasignar todo con un arreglo vacío, sin insertar nada', async () => {
      ...
      await assignMemberPermissions.execute('owner-1', 'member-1', []);
    ...
    /**
     * Sin colapsar los repetidos, la tabla de asignaciones aceptaría dos filas del mismo
     * permiso para el mismo miembro.
     */
    it('colapsa los ids repetidos antes de validar y de escribir', async () => {
      accountRepository.findOne
        .mockResolvedValueOnce({ id: 'member-1', organizationId: 'org-1' })
        .mockResolvedValueOnce(adminMembership());
      organizationPermissionRepository.find.mockResolvedValue([
      ...
      expect(result.data.permissionIds).toEqual(['perm-1']);
    });
    ...
    it('lanza ForbiddenException si el llamador no es ADMIN de esa organización', async () => {
      ...
      await expect(
        assignMemberPermissions.execute('intruder', 'member-1', []),
      ).rejects.toThrow(ForbiddenException);
      expect(dataSource.transaction).not.toHaveBeenCalled();
    });
  });
