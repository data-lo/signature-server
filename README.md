...
- **`POST /api/v1/organizations/invite` no invita a nadie todavía — es intencional (alcance delimitado de su historia)**: no envía correo, no genera un token de invitación/aceptación, y no inserta ninguna fila (`AccountMemberEntity` con `isActive: false`/pendiente, entrada en el catálogo de Redis). Hoy responde éxito apenas valida el payload y los permisos del llamador. La siguiente iteración necesita decidir: modelo de invitación pendiente (¿una fila en `account_members` con un estado "invited" antes de "active"? ¿una tabla nueva?), quién puede aceptar (¿el email debe corresponder a un usuario ya registrado, o el flujo también cubre invitar a alguien sin cuenta?), expiración del token, y la plantilla de correo (`EmailService` ya existe y se usa para otros flujos).
