...
- **Cobertura de tests parcial**: 109 tests en 21 suites. `user.service.spec.ts`, `document.service.spec.ts`, `account.service.spec.ts` y `account-member.service.spec.ts` ya cubren comportamiento real (éxito + errores), igual que `account.controller.spec.ts`/`account-member.controller.spec.ts`/`document.controller.spec.ts` (delegación con el userId/accountId del JWT/header). El resto de specs (`audit`, etc.) siguen siendo smoke tests (`should be defined`). Tampoco hay un e2e que ejercite el flujo completo registro→login→onboarding→crear organización→crear documento (`test/app.e2e-spec.ts` sigue siendo el scaffold por defecto de Nest).
