...
- **`EventModule` sin conectar a ningún productor real**: `EventEntity`/`EventService` (ver "Resuelto en esta ronda" del diagrama ER-V2 más arriba) solo modelan la persistencia — nada llama a `EventService.create()` todavía. Falta decidir e implementar dónde engancharlo: lo más directo es dentro de `DocumentEventsProducer`/`OrganizationInvitationEventsProducer` (justo antes o después de emitir a Kafka), para que cada evento de dominio que ya se publica también quede registrado en Postgres para trazabilidad/consulta. No se hizo en la misma ronda porque toca cada call site de cada producer existente — alcance mayor al de solo crear el módulo/entidad que pedía el ticket.
