...
- **Los checks de autorización siguen siendo "es rol ADMIN", no permisos granulares por resource+action**: `assertIsAccountAdmin` (implementado por separado en `account.service.ts` y en `account-member.service.ts`) ya lee `roleId`/`role.name` vía el catálogo RBAC en vez del enum viejo, pero sigue comparando contra un nombre de rol fijo (`ADMIN`), no contra `role_permissions` (p. ej. "tiene permiso `DOCUMENT:DELETE`"). Los 12 permisos de ADMIN y los 2 de MEMBER que pobló el seed no se consultan todavía en ningún guard — es la siguiente capa de granularidad si se necesita (p. ej. un rol custom de organización con permisos parciales).
- **Migración de modelo completa (`ENTIDAD_RELACIÓN_V2`)**: `Organization` separada de `OrganizationDetail`, mover `email`/`password` de `Users` a `Account`, `Collaborator` reemplazando `DocumentParticipant` con campos nuevos (`comments`, `geoLoc`, `visibilityLevel`, `cancellationReason`, `reminderPeriodicity`, `signatureType`), `Watcher`/`Notification`/`Event`, `verification_code`, `SimpleSignature`/`FIELSignature`, `Document Transaction`. **Nota**: ya se implementó una porción acotada de multi-cuenta (creación de organización, catálogo de cuentas cacheado en Redis, y ahora el catálogo RBAC real conectado a `AccountMemberEntity.roleId` — ver "Resuelto en esta ronda" más abajo) — pero sigue siendo un paso intermedio, no la migración completa. Sigue sin tocarse el login/JWT (`sub`/`email`/`roles`/`nationalId`, sin claims de cuenta/organización activa).
...
- **Cobertura de tests parcial**: 118 tests en 23 suites. `user.service.spec.ts`, `document.service.spec.ts`, `account.service.spec.ts`, `account-member.service.spec.ts` y `roles.service.spec.ts` ya cubren comportamiento real (éxito + errores, incluyendo `NotFoundException` cuando `create()`/`update()` de `account-member` reciben un `roleId` inexistente), igual que `account.controller.spec.ts`/`account-member.controller.spec.ts`/`document.controller.spec.ts`/`roles.controller.spec.ts` (delegación con el userId/accountId del JWT/header). El resto de specs (`audit`, etc.) siguen siendo smoke tests (`should be defined`). Tampoco hay un e2e que ejercite el flujo completo registro→login→onboarding→crear organización→crear documento (`test/app.e2e-spec.ts` sigue siendo el scaffold por defecto de Nest).
