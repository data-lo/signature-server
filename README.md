...
### `account-member` (`/account-member`) — membresías (JWT, solo OWNER)

`AccountMemberService`: `create()` (otorgar acceso con uno o más roles), `findByAccount()`, `findOne()`, `update()` (cambia rol/puesto/vigencia), `remove()` (revocación = soft delete `isActive=false`). Los 5 exigen que el llamador sea un OWNER **activo** de la cuenta involucrada (`ForbiddenException` si no) — para `findOne()`/`update()`/`remove()`, que reciben el id de la membresía (no de la cuenta), primero se resuelve la membresía para obtener su `accountId` y luego se valida contra ese id.

También expone `assertIsActiveMember(userId, accountId)` (público, sin controlador propio): check de tenant más laxo — cualquier miembro **activo** basta, sin importar el rol — usado por otros módulos que necesitan validar el header `X-Account-Id` contra una membresía real (hoy solo `document`, ver su sección arriba).

### `roles` (`GET /api/v1/roles`, JWT) — catálogo RBAC (Role/Resource/Action/Permission/RolePermission)

Módulo nuevo, independiente del `role[]` (enum-array) de `AccountMemberEntity` — **todavía no se integran entre sí** (ver Pendientes). Centraliza las 5 entidades de control de acceso: `RoleEntity`, `ResourceEntity`, `ActionEntity`, `PermissionEntity`, `RolePermissionEntity` (tabla pivote).

`RolesService.findAllSystemRoles()`: `roleRepository.find({ where: { isSystemRole: true } })`, ordenado por `name`. `RolesController` expone `GET /api/v1/roles` (JWT, sin check de ownership — es un catálogo de solo lectura, no datos de una cuenta concreta) devolviendo `{id, name, isSystemRole}` por rol; pensado para poblar el modal de invitar miembros en el frontend.
...
- **El nuevo `RolesModule` (Role/Resource/Action/Permission/RolePermission) todavía no se usa para autorizar nada**: hoy es un catálogo de solo lectura (`GET /api/v1/roles`) pensado para poblar UI (el modal de invitación de miembros). Ningún guard/servicio consulta `role_permissions` para decidir si un usuario puede hacer algo — todos los checks de ownership actuales (`assertIsOwner`/`assertIsActiveMember`) siguen leyendo directo el `role[]` (enum-array) de `AccountMemberEntity`, que es un sistema **paralelo y sin relación** con este módulo nuevo. Conectarlos (que invitar a alguien con un `roleId` real cree también su `AccountMemberEntity` correspondiente, y que los guards consulten `permissions` en vez del enum) es justo la integración que falta y que motiva el siguiente punto.
- **Migración de modelo completa (`ENTIDAD_RELACIÓN_V2`)**: `Organization` separada de `OrganizationDetail`, mover `email`/`password` de `Users` a `Account`, `Collaborator` reemplazando `DocumentParticipant` con campos nuevos (`comments`, `geoLoc`, `visibilityLevel`, `cancellationReason`, `reminderPeriodicity`, `signatureType`), `Watcher`/`Notification`/`Event`, `verification_code`, `SimpleSignature`/`FIELSignature`, `Document Transaction`. **Nota**: ya se implementó una porción acotada de multi-cuenta (creación de organización, catálogo de cuentas cacheado en Redis, membresías con rol nullable, y ahora el catálogo RBAC básico — ver "Resuelto en esta ronda" más abajo) — pero sigue siendo un paso intermedio, no la migración completa. Sigue sin tocarse el login/JWT (`sub`/`email`/`roles`/`nationalId`, sin claims de cuenta/organización activa).
...
- **Migraciones `MakeAccountMemberRoleNullable` y `AddAccountIdToDocuments` sin confirmar contra una base con datos reales**: ambas se generaron y se verificaron contra el esquema de desarrollo, pero no contra un ambiente con datos de producción reales. `AddAccountIdToDocuments` en particular hace un backfill (asigna la cuenta PERSONAL del creador a cada documento existente) y falla a propósito si algún documento no encuentra esa cuenta — hay que correrla primero en staging y confirmar que no truene antes de un despliegue real. `migrationsRun: true` las aplica solas al desplegar, no requiere acción manual salvo esa verificación previa.
...
- **Cobertura de tests parcial**: 114 tests en 23 suites. `user.service.spec.ts`, `document.service.spec.ts`, `account.service.spec.ts`, `account-member.service.spec.ts` y `roles.service.spec.ts` ya cubren comportamiento real (éxito + errores), igual que `account.controller.spec.ts`/`account-member.controller.spec.ts`/`document.controller.spec.ts`/`roles.controller.spec.ts` (delegación con el userId/accountId del JWT/header). El resto de specs (`audit`, etc.) siguen siendo smoke tests (`should be defined`). Tampoco hay un e2e que ejercite el flujo completo registro→login→onboarding→crear organización→crear documento (`test/app.e2e-spec.ts` sigue siendo el scaffold por defecto de Nest).
...
- **Explícitamente fuera de esta ronda**: ninguna autorización real usa todavía estas tablas (ver Pendientes) — es solo el catálogo + su seed, tal como pedía la historia.
